...
var (
	...
	repo                                factorial.TokenRepository
)
...
func HomeHandler(w http.ResponseWriter, r *http.Request) {
	...
	token, err := repo.GetToken(uuid.Nil)
	if err != nil {
		log.Panicln("Error while getting token", err)
	}
...
func FactorialOAuthCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := repo.SaveToken(uuid.Nil, token); err != nil {
		log.Panicln("Error while saving token", err)
	}
